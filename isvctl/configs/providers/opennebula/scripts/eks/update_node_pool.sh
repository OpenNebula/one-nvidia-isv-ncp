...
if [ -z "$GROUP_ID" ]; then
    echo "Error: Node pool ${POOL_NAME} not found" >&2
    exit 1
fi

echo "Scaling group $POOL_NAME ($GROUP_ID) to $DESIRED_SIZE..." >&2
oneks scale group "$GROUP_ID" --target "$DESIRED_SIZE" >&2
...
echo "$NODES" | while read -r node; do
    ...
    echo "$TAINTS_JSON" | jq -c '.[]' | while read -r taint; do
        ...
        if [ -n "$VALUE" ] && [ "$VALUE" != "null" ]; then
            ...
            $KUBECTL taint nodes "$node" "${KEY}:${EFFECT}" --overwrite >&2
        fi
    done
done
...
# Output Payload
# -----------------------------------------------------------------------------
# Flavour is usually fixed or inferred for these stubs
