...
def main() -> int:
    parser = argparse.ArgumentParser(description="Start OpenNebula instance")
    ...
    try:
        ...
        # Resume the VM
        one.vm.action("resume", args.instance_id)
        ...
        if not running:
            raise RuntimeError("Timeout waiting for VM to start")
        ...
        if args.public_ip and args.key_file:
            ...
            if not ssh_ready:
                raise RuntimeError("SSH not ready after VM start")
