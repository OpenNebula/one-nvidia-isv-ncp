...
"""Describe an OpenNebula bare-metal instance through OpenNebula and NICo."""
...
def map_state(state: int, lcm_state: int, nico_status: str | None = None) -> str:
    """Map OpenNebula and NICo states to the provider-neutral instance contract."""
    if state == 3 and lcm_state == 3 and nico_status == "Ready":
        return "running"
    ...
    if state == 6:
        return "terminated"
    if state in {7, 11} or nico_status in {"Error", "Failed"}:
        return "failed"
    return "unknown"
...
def main() -> int:
    """Describe an OpenNebula bare-metal instance and return its current state."""
    parser = argparse.ArgumentParser(description="Describe OpenNebula bare-metal instance")
    ...
    try:
        ...
        nico_status = str(monitoring.get("NICO_STATUS") or "")
        if deploy_id:
            nico_instance = NicoAPI(args.api_timeout).get_instance(deploy_id)
            nico_status = str(nico_instance.get("status") or nico_status)
        ...
        result["state"] = map_state(state, lcm_state, nico_status)
        result["opennebula_state"] = state
        result["opennebula_lcm_state"] = lcm_state
        result["nico_status"] = nico_status or None
